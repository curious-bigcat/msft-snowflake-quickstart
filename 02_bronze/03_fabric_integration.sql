...
-- Verify metadata locations in OneLake:
...
--
-- Run after Section A and Section B.
...
UNION ALL SELECT 'BEFORE', 'RAW_CUSTOMERS_ICEBERG',    COUNT(*) FROM ICEBERG.RAW_CUSTOMERS_ICEBERG
UNION ALL SELECT 'BEFORE', 'RAW_ORDERS_ICEBERG',       COUNT(*) FROM ICEBERG.RAW_ORDERS_ICEBERG;
...
-- -----------------------------------------------------------------------------
-- C3. Incremental rows in ICEBERG.RAW_* write-back tables (Section B)
-- Each INSERT adds new Parquet files to OneLake — Fabric shortcuts reflect
-- the new data without any manual refresh.
-- -----------------------------------------------------------------------------
USE SCHEMA ICEBERG;

INSERT INTO ICEBERG.RAW_CUSTOMERS_ICEBERG
SELECT CUSTOMER_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE,
       ADDRESS, CITY, STATE, COUNTRY, POSTAL_CODE,
       CUSTOMER_SEGMENT, REGISTRATION_DATE, IS_ACTIVE,
       CURRENT_TIMESTAMP() AS EXPORTED_AT
FROM BRONZE.CUSTOMERS
WHERE REGISTRATION_DATE >= DATEADD('day', -30, CURRENT_DATE())
LIMIT 200;

INSERT INTO ICEBERG.RAW_ORDERS_ICEBERG
SELECT ORDER_ID, CUSTOMER_ID, ORDER_DATE, ORDER_STATUS,
       TOTAL_AMOUNT, DISCOUNT_AMOUNT, SHIPPING_AMOUNT,
       PAYMENT_METHOD, REGION, CHANNEL, SOURCE_SYSTEM,
       CURRENT_TIMESTAMP() AS EXPORTED_AT
...
UNION ALL SELECT 'AFTER', 'RAW_CUSTOMERS_ICEBERG',    COUNT(*) FROM ICEBERG.RAW_CUSTOMERS_ICEBERG
UNION ALL SELECT 'AFTER', 'RAW_ORDERS_ICEBERG',       COUNT(*) FROM ICEBERG.RAW_ORDERS_ICEBERG;
...
--   SELECT COUNT(*) FROM raw_customers_iceberg; -- should match AFTER RAW_CUSTOMERS_ICEBERG
--   SELECT COUNT(*) FROM raw_orders_iceberg;    -- should match AFTER RAW_ORDERS_ICEBERG
