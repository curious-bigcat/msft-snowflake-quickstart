-- =============================================================================
-- MSFT-SNOWFLAKE QUICKSTART LAB: Synthetic Data Generation
-- =============================================================================
-- Generates realistic synthetic data for all RAW tables.
-- Prerequisites: Run 02_native_data/01_create_tables.sql first.
-- =============================================================================
...
SELECT
    ...
    CONCAT('High-quality ', LOWER(CATEGORY), ' product designed for enterprise and business use. ',
           'Features include advanced performance, reliability, and comprehensive support.') AS DESCRIPTION,
...
SELECT
    ...
    DATEADD('second',
        -UNIFORM(0, 63072000, RANDOM()),  -- random seconds in ~2 years
        CURRENT_TIMESTAMP()
    ) AS ORDER_DATE,
...
INSERT INTO RAW.ORDER_ITEMS (ORDER_ID, PRODUCT_ID, QUANTITY, UNIT_PRICE, LINE_TOTAL, DISCOUNT_PCT)
WITH order_ids AS (
...
expanded AS (
    ...
    FROM order_ids o,
         TABLE(GENERATOR(ROWCOUNT => 3)) g  -- up to 3 items per order
    WHERE UNIFORM(1,100,RANDOM()) <= 80  -- ~80% chance for each item slot
)
SELECT
    ORDER_ID,
...
-- =============================================================================
-- 5. PRODUCT REVIEWS — ~15,000 reviews with varied text
-- =============================================================================

INSERT INTO RAW.PRODUCT_REVIEWS (
    ...
    CASE
        ...
        WHEN UNIFORM(1,100,RANDOM()) <= 30 THEN
            CONCAT(
                ...
                ' ',
                ARRAY_CONSTRUCT(
                    'The build quality is top-notch and it integrates seamlessly with our existing systems.',
                    ...
                    'The features are exactly what we needed for our digital transformation project.',
                    'Works perfectly with Microsoft 365 and our Azure infrastructure.'
        ...
        WHEN UNIFORM(1,100,RANDOM()) <= 65 THEN
            CONCAT(
                ARRAY_CONSTRUCT(
                    'Good product overall, meets our requirements.',
                    'Solid performance with a few minor issues.',
                    'Very satisfied with this purchase.',
                    'Works well for our team, good value.',
                    'Reliable product with room for improvement.'
                )[UNIFORM(0,4,RANDOM())]::VARCHAR,
                ' ',
                ARRAY_CONSTRUCT(
                    'The documentation could be more detailed but the product itself is great.',
                    ...
                    'Good product but the learning curve is steeper than expected.',
                    'Performs well under load. The admin console could use some polish.'
                )[UNIFORM(0,4,RANDOM())]::VARCHAR
            )
        -- 3-star reviews (20%)
        WHEN UNIFORM(1,100,RANDOM()) <= 85 THEN
            CONCAT(
                ARRAY_CONSTRUCT(
                    ...
                    'It works but does not stand out.',
                    'Meets basic requirements but lacks advanced features.',
                    'OK for the price point.'
                )[UNIFORM(0,4,RANDOM())]::VARCHAR,
                ' ',
                ARRAY_CONSTRUCT(
                    'The performance is adequate but not impressive compared to competitors.',
                    'Some features are missing that we expected at this price point.',
                    'Setup was more complex than the documentation suggested.',
                    'Works for basic use cases but struggles with our enterprise workload.',
        ...
        WHEN UNIFORM(1,100,RANDOM()) <= 95 THEN
            CONCAT(
                ARRAY_CONSTRUCT(
                    ...
                    'Quality is below what was advertised.',
                    'Would not purchase again.',
                    'Needs significant improvement.'
                )[UNIFORM(0,4,RANDOM())]::VARCHAR,
                ' ',
                ARRAY_CONSTRUCT(
                    'We encountered multiple issues during setup and the support team was slow to respond.',
                    'The product crashed several times during our testing phase.',
                    'Performance degrades significantly under heavy load.',
                    'Missing critical features that were listed in the product description.',
                    'Compatibility issues with our existing infrastructure caused major delays.'
                )[UNIFORM(0,4,RANDOM())]::VARCHAR
            )
        -- 1-star reviews (5%)
        ELSE
            CONCAT(
                ARRAY_CONSTRUCT(
                    ...
                    'The worst purchase our team has made.',
                    'Returned immediately after testing.',
                    'Absolutely unacceptable quality.'
                )[UNIFORM(0,4,RANDOM())]::VARCHAR,
                ' ',
                ARRAY_CONSTRUCT(
                    'The product stopped working after just one week. Support has been unresponsive.',
                    'Data loss occurred during a routine operation. This is unacceptable for enterprise use.',
                    'Security vulnerabilities were discovered during our audit. Removed immediately.',
                    'Completely incompatible with our systems despite claiming full integration support.',
                    'We lost significant productivity due to constant crashes and bugs.'
                )[UNIFORM(0,4,RANDOM())]::VARCHAR
            )
    END AS REVIEW_TEXT,
...
SELECT
    ...
    -- Ticket subject based on category
    CASE cat.CATEGORY
        WHEN 'Technical' THEN
            ARRAY_CONSTRUCT(
                'Cannot connect to cloud service',
                'Software crashes on startup',
                'Integration error with Azure AD',
        ...
        WHEN 'Billing' THEN
            ARRAY_CONSTRUCT(
                ...
                'Requesting license upgrade',
                'Duplicate billing for subscription',
        ...
        WHEN 'Shipping' THEN
            ARRAY_CONSTRUCT(
                ...
                'Need expedited shipping upgrade',
                'Tracking number not working',
                'Return shipping label request'
            )[UNIFORM(0,5,RANDOM())]::VARCHAR
        WHEN 'Product Defect' THEN
            ARRAY_CONSTRUCT(
                'Hardware malfunction out of box',
                'Screen flickering on new monitor',
                'Battery not holding charge',
                'Keyboard keys sticking',
                'Device overheating under normal use',
    ...
    CONCAT(
        ...
        ARRAY_CONSTRUCT(
            'The system is not responding as expected after the latest update.',
            ...
            'This is a recurring issue that we reported previously.'
        )[UNIFORM(0,7,RANDOM())]::VARCHAR,
        ' Environment: ',
        ARRAY_CONSTRUCT('Production','Staging','Development','Testing')[UNIFORM(0,3,RANDOM())]::VARCHAR,
        '. Impact: ',
        ARRAY_CONSTRUCT('All users affected','Limited to specific team','Single user','Intermittent for all')
            [UNIFORM(0,3,RANDOM())]::VARCHAR,
        '.'
    ) AS TICKET_DESCRIPTION,

    -- Priority distribution: Critical 5%, High 15%, Medium 50%, Low 30%
    CASE
    ...
    -- Status distribution: Open 15%, In Progress 20%, Resolved 40%, Closed 25%
    CASE
    ...
    CASE
        WHEN UNIFORM(1,100,RANDOM()) <= 5  THEN ROUND(UNIFORM(2, 24, RANDOM())::NUMERIC(10,2), 2)
        WHEN UNIFORM(1,100,RANDOM()) <= 20 THEN ROUND(UNIFORM(4, 48, RANDOM())::NUMERIC(10,2), 2)
        WHEN UNIFORM(1,100,RANDOM()) <= 70 THEN ROUND(UNIFORM(8, 120, RANDOM())::NUMERIC(10,2), 2)
        ELSE ROUND(UNIFORM(24, 240, RANDOM())::NUMERIC(10,2), 2)
    ...
    CASE
        WHEN UNIFORM(1,100,RANDOM()) <= 65 THEN  -- 65% resolved
            DATEADD('hour',
                UNIFORM(2, 240, RANDOM()),
